...
- Production evidence must include numeric provider score, score kind, rank, exact source/version binding, canonical source URL and title, retrieved heading path, normalized page heading outline, safe source-derived CTA candidates, and retrieval configuration identity. Reject scoreless or off-manifest results and represent insufficient evidence as an empty typed outcome.
