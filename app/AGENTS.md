...
- Website ingestion uses Firecrawl map followed by a bounded crawl. Publication requires every normalized mapped URL to have non-empty extracted Markdown in the staged version.
...
- For Phase 2A, prove map/crawl manifest completeness, idempotent document uploads, provider-source binding, atomic publication, rollback, and rejection of retrieval hits outside the published manifest.
