...
- The Webpages input accepts either a complete website or one exact page. Complete websites use Firecrawl Map, pre-scrape operational-route filtering, and exact batch scrape of the remaining pages as one user Add command. Exact pages use Firecrawl Scrape directly. An explicit row Re-run always scrapes that page with `maxAge: 0`.
