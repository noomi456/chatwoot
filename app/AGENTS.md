...
- In the current single-deployment architecture, a CE extension schedules at most one `ChatRing::AiTurn` internally after native message-template handling and records the trigger's native-handling snapshot. The managed account-owned AgentBot remains the native owner and sender but has no public self-webhook, and the managed webhook route rejects requests in internal mode. Timestamped HMAC ingress and the conditional HTTP endpoint are inactive, isolated contracts for a future physically external runtime; that mode must use both boundaries end to end.
...
- ChatRing records one durable idempotent customer-affecting commit per `AiTurn`, covering reply or handoff. Retries reuse its stable key and an already committed key returns the original result. The compile-time public-response gate remains authoritative until the v2.2 full native-lifecycle suite passes.
