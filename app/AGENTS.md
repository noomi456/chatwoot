...
- Managed Assistant webhook ingress must verify Chatwoot's exact timestamped `sha256=` HMAC over the raw body, enforce a five-minute replay window, validate Account/Inbox/AgentBot scope, and durably deduplicate delivery IDs before creating at most one `ChatRing::AiTurn` for a customer message. Webhook payload state is never final authority; processing re-reads Chatwoot records.
- `ChatRing::AiTurn` may record verified work before the Brain exists, but webhook processing must not call an LLM or create a public Message. Public AI remains compile-time disabled until the shared serialization and Section 21.7 release gates pass.
...
- Every public incoming customer message and public human reply acquires the Conversation row lock before insert. Conditional managed-AgentBot replies use that same lock while rechecking turn, binding, Assistant version, ownership, freshness, and idempotency before creating an ordinary Chatwoot Message.
- ChatRing records one `OutboundCommit` per `AiTurn`; retries reuse its stable key and an already committed key returns the original Message. The compile-time public-response gate remains authoritative until the controlled Section 21.7 concurrency suite passes.
