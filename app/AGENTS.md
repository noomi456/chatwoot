...
- The Webpages input accepts either a complete website or one exact page. Complete websites use Firecrawl Map, pre-scrape operational-route filtering, user page selection, and exact batch scrape. Exact pages use Firecrawl Scrape directly. An explicit row Re-run always scrapes that page with `maxAge: 0`.
...
- Successful extraction updates the selected material snapshot and queues exactly one hidden replacement index from all active Training Materials. A manifest digest prevents duplicate builds and prevents an older build from replacing newer user changes.
