...
- In the current single-deployment architecture, the PR #17 CE extension observes synchronous native message-template handling while the public gate is closed. Before inference can be enabled, a directly stacked native-handling completion contract must combine that observation with actual immediate Automation completion/effects and release at most one `ChatRing::AiTurn`. The managed account-owned AgentBot remains the native owner and sender but has no public self-webhook, and the managed webhook route rejects requests in internal mode. Timestamped HMAC ingress and the conditional HTTP endpoint are inactive, isolated contracts for a future physically external runtime; that mode must use both boundaries end to end.
...
- For the Assistant response loop, prove the v2.2 Section 13 native template/automation precedence, internal scheduling, managed assignment/rebind/handoff, scoped serialization, failure fallback, idempotency, normal `SendReplyJob` delivery, non-managed channel regression, and identical persisted output in the Inbox and Classic Widget.
