...
const currentBinding = computed(() =>
  selectedAssistant.value?.bindings?.find(item => item.status === 'active')
);
const currentInbox = computed(() =>
  websiteInboxes.value.find(
    inbox => inbox.id === Number(currentBinding.value?.inbox_id)
  )
);
...
const loadTurns = async assistantId => {
  const response = await ChatRingAssistantsAPI.turns({
...
const loadAssistants = async () => {
  ...
  try {
    ...
    if (assistants.value.length) await selectAssistant(assistants.value[0]);
  } catch (error) {
    useAlert(apiError(error));
  } finally {
    isLoading.value = false;
...
const disableBinding = async () => {
  ...
  isOperating.value = true;
  try {
    await ChatRingAssistantsAPI.disableBinding(currentBinding.value.id);
    const response = await ChatRingAssistantsAPI.show(
      selectedAssistant.value.id
    );
    replaceAssistant(response.data);
          ...
          <div v-if="activePanel === 'configuration'" class="grid gap-6 mt-6">
            ...
            >
              <Input
                ...
                :disabled="isArchived || !!selectedAssistant.current_version"
              />
              <Input
              ...
              <div class="flex justify-end gap-3">
                <Button
                  ...
                  :is-loading="isSaving"
                  :disabled="isArchived || hasStaleDraft"
                  @click="saveDraft"
                />
                <Button
                  :label="t('CHATRING_ASSISTANTS.PUBLISH')"
                  :is-loading="isOperating"
                  :disabled="isArchived || hasStaleDraft || isDraftDirty"
                  :title="
                    isDraftDirty
            ...
            >
              ...
              <p class="text-sm text-n-slate-12">
                {{
                  currentInbox
                    ? t('CHATRING_ASSISTANTS.CONNECTED', {
                        inbox: currentInbox.name,
                      })
                    : t('CHATRING_ASSISTANTS.NOT_CONNECTED')
                }}
              </p>
              <div v-if="!isArchived" class="flex items-end gap-3">
                ...
                <Button
                  ...
                  :is-loading="isOperating"
                  :disabled="!selectedInboxId || !releaseReady"
                  @click="checkInbox"
                />
              </div>
              <div
              ...
              <Button
                v-if="currentBinding && !isArchived"
