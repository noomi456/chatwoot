...
Own the private DocsGPT API and ingestion worker used by ChatRing's account Knowledge Base.
...
- Require both the shared `DOCSGPT_INTERNAL_KEY` and an independently signed ChatRing-to-DocsGPT service credential for retrieval and mutation operations, scoped to the account, hidden knowledge index, content binding, operation, and provider source. Private Docker networking is not authentication.
...
- Keep DocsGPT source pinned to `616e6fe9c435bbc6bb472636db6b3ee2b9bcaf66`. Apply only the reviewed native pgvector correctness backport from `795e39a6bc40ea499f92290ac19540a842713841` plus its required probe setting: do not auto-create IVFFlat on an empty table, detect legacy indexes without caching absence, apply `sqrt(lists)` probes, and fall back to exact search when filtered ANN under-returns.
...
- Confirm the active account index retrieves only current, non-deleted Training Materials in its stored manifest.
