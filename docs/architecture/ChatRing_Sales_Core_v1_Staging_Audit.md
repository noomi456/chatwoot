...
Locked v1 choices are the complete Web Widget/RealtimeKit sales vertical first; Twilio SMS, Email, WhatsApp Cloud, Facebook Messenger and Instagram then certified independently when available; and Twilio for PSTN Voice. WhatsApp support requires a configured Cloud Inbox; a deployed default/non-Cloud Inbox is not silently claimed. This is static source/product verification, not runtime certification. Deployed credentials, Meta app approval, phone-number capabilities and every provider path still require runtime audit in the implementation PR that uses them.
