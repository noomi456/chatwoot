...
- ChatRing is Sales-first. The complete Web Widget vertical is the first release authority. Native Chatwoot Ongoing Campaigns own proactive Engagement bubble messages, wait-time triggers and URL targeting; ChatRing extends that native Campaign rule only where page-scroll percentage is genuinely missing. Static Conversation Starters are a separate Inbox configuration: up to four admin prompts, two shown before a conversation, whose clicks create ordinary Widget customer Messages. Tools are configured and selected inside Playbooks/Brain, while their server-side policy and execution ledgers remain bounded authorization/idempotency records rather than a separate product page. A grounded free-form response may contain one native Message, up to two suggested questions and one expiring evidence-linked Microsite artifact; missing evidence omits the suggestions or artifact rather than fabricating them.
- Sales Core v1 completes and certifies the full Web Widget sales-and-RealtimeKit vertical first. The remaining approved native text-provider paths are Twilio SMS, Email, WhatsApp Cloud, Facebook Messenger and Instagram; each is certified independently when its real credentials, approvals, compliance and test environment are ready rather than in a fixed architectural order. Twilio is the selected SMS/PSTN provider; Telnyx, Bandwidth SMS, Twilio WhatsApp, API, Telegram, LINE, TikTok and X/Twitter are outside the approved v1 certification set unless a later written product decision adds them. Cloudflare RealtimeKit is the selected native Website human-call seam.
- Development/pre-production model execution uses `gpt-5.4` with the credential retained only in the deployment secret store. Human escalation reuses native Inbox hours plus eligible online Inbox members/assignment policy; an hours-only check must never produce a false transfer claim. When a native human assignment cannot be completed, preserve the request in Chatwoot and use the channel-neutral appointment/callback Tool. Playbook side questions return to the exact pending step, while repeated explicit human requests follow this same native handoff/availability rule. A certified Website surface may embed the Inbox-approved calendar; other channels use approved links or forms.
...
- Product/UX donor code may be clean-copied only after file-level provenance, license, tenancy, security and native-boundary review. Copying is limited to implementation-neutral UI/value-contract pieces that can be rewired to native Chatwoot authority; never copy donor conversation stores, transports, provider credentials, orchestration, Supabase tenancy or AI-voice runtime. The Enterprise source remains excluded even when it implements a desired capability.
