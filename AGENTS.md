...
- Sales Core v1 certifies the Web Widget first, then the approved native text-provider paths: Twilio SMS, Email, WhatsApp Cloud, Facebook Messenger and Instagram when the real provider configuration is available. Twilio is the selected SMS/PSTN provider; Telnyx, Bandwidth SMS, Twilio WhatsApp, API, Telegram, LINE, TikTok and X/Twitter are outside the approved v1 certification set unless a later written product decision adds them. Cloudflare RealtimeKit is the selected native Website human-call seam.
