...
- The `ChatRing::AssistantSpike` compile-time gate remains disabled while the production Assistant response loop is incomplete. It must not be opened until the v2.2 native-lifecycle release gates pass on the exact deployed path.
...
- Public AI responses remain disabled until `docs/architecture/ChatRing_AI_Chatwoot_Integration_Contract_v2.2.md` Section 13 passes. For the current single deployment, ChatRing schedules turns internally after native template handling; the account-owned AgentBot remains the Chatwoot owner/sender and must not public-webhook back into the same Rails application.
- Native Chatwoot lifecycle is always authoritative. Before creating a ChatRing lifecycle, transport, assignment, message writer, handoff, retry or delivery mechanism, identify and extend the equivalent native Chatwoot service first. Add a ChatRing-only component only when Chatwoot lacks the capability, and keep it subordinate to native ownership, state, mutation and delivery.
