...
RSpec.describe ChatRing::Brain::DecisionSchema do
  it 'uses OpenAI strict-schema-compatible nullable fields' do
    ...
    expect(schema.fetch('required')).to contain_exactly(
      'decision_type', 'response_text', 'reason_code', 'evidence_ids', 'tool_request', 'playbook_control'
    )
    ...
    expect(tool.fetch('required')).to contain_exactly('key', 'version', 'arguments')
    expect(tool.dig('properties', 'arguments', 'required')).to contain_exactly('requested_time_window', 'reason_code')

    playbook = schema.dig('properties', 'playbook_control', 'anyOf').first
    expect(playbook.fetch('required')).to contain_exactly('action', 'answer_value')
  end
end
